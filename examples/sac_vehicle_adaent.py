...
from policy.sac_adaent import SACPolicy
...
def get_args():
    ...
    parser.add_argument("--logdir", type=str, default="log/Vehicle")
    parser.add_argument("--algo-name", type=str, default="sac-AdaEnt")
    ...
    parser.add_argument("--entropy-n-sample", type=int, default=100)
    parser.add_argument("--entropy-dist-threshold", type=float, default=0.8)
...
def test_sac(args=get_args()):
    ...
    critic1_s = Critic(net_c1_s, device=args.device).to(args.device)
    critic1_s_optim = torch.optim.Adam(critic1_s.parameters(), lr=args.critic_lr)
    critic2_s = Critic(net_c2_s, device=args.device).to(args.device)
    critic2_s_optim = torch.optim.Adam(critic2_s.parameters(), lr=args.critic_lr)
    ...
    policy = SACPolicy(
        ...
        critic2_s,
        critic2_s_optim,
        entropy_n_sample=args.entropy_n_sample,
        entropy_dist_threshold=args.entropy_dist_threshold,
        tau=args.tau,
        gamma=args.gamma,
        alpha=args.alpha,
    ...
    if args.resume_path:
        ...
        policy.critic1_s.load_state_dict(ckpt["critic1_s"])
        policy.critic2_s.load_state_dict(ckpt["critic2_s"])
        ...
        critic1_s_optim.load_state_dict(ckpt['critic1_s_optim'])
        critic2_s_optim.load_state_dict(ckpt['critic2_s_optim'])
    ...
    def save_best_fn(policy):
        ...
        state = {
            ...
            "critic1_s": policy.critic1_s.state_dict(),
            "critic2_s": policy.critic2_s.state_dict(),
            "critic1_s_optim": critic1_s_optim.state_dict(),
            "critic2_s_optim": critic2_s_optim.state_dict(),
            "alpha": policy._alpha
        }
