...
def get_args():
    ...
    parser.add_argument("--conditioned-sigma", type=int, default=False)
    parser.add_argument("--sigma-param", type=float, default=-1)
    parser.add_argument("--algo-name", type=str, default="ppo")

    parser.add_argument("--task", type=str, default="QuadRotor-v0")

    parser.add_argument("--action-unbounded", type=int, default=True)
    parser.add_argument("--action-scaling", type=int, default=False)
    parser.add_argument("--bound-action-method", type=str, default="")


    parser.add_argument("--gae-lambda", type=float, default=0.95)
    parser.add_argument("--rew-norm", type=int, default=False)
    parser.add_argument("--obs-norm", type=bool, default=False)
...
def test_ppo(args=get_args().parse_args()):
    env, train_envs, test_envs = make_custom_env(
        args.task, args.seed, args.training_num, args.test_num, obs_norm=args.obs_norm
    )
    ...
    if not args.conditioned_sigma:
        torch.nn.init.constant_(actor.sigma_param, args.sigma_param)
    ...
    def save_checkpoint_fn(epoch, env_step, gradient_step):
        ...
        if args.obs_norm:
            save_dict["obs_rms"] = train_envs.get_obs_rms()
    ...
    result = test_collector.collect(n_episode=1, render=0)
